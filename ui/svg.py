...
• نقاط اتصال (brass pins) بين الأحجار
• تموضع ديناميكي يتّسع مع عدد الأحجار
"""
...
class SVGRenderer:
    ...
    HW  = TW // 2      # نصف العرض
    PR  = 6            # نصف قطر النقطة
    SP  = 8            # فراغ أحجار اليد (backwards compat)
    HAND_GAP  = 8      # فراغ بين أحجار اليد
    CHAIN_GAP = 0      # ★ بدون فراغ في السلسلة — متصلة!
    ...
    def _draw_chain_tile(tile, x, y, vertical=False):
        ...
        الزوايا مُصغّرة (rx=4) لتقليل الفراغ عند الالتصاق.
        """
        TW  = SVGRenderer.TW
        TH  = SVGRenderer.TH
        HW  = SVGRenderer.HW
        ...
        if vertical:
            # ── دبل عمودي: عرض=TH(50)  ارتفاع=TW(100) ──
            vw   = TH
            vh   = TW
            half = vh // 2
            ...
            # ── عادي أفقي: عرض=TW(100)  ارتفاع=TH(50) ──
            s  = f'<g transform="translate({x},{y})">\n'
    ...
    # ════════════════════════════════════
    #  ★ رسم نقطة الاتصال (Brass Pin)
    # ════════════════════════════════════

    @staticmethod
    def _draw_connector(x, y):
        """نقطة اتصال نحاسية بين حجرين متجاورين"""
        return (
    ...
    def board(board, w=900, h=180):
        ...
        • الدبل عمودي (عمودي على السلسلة)
        • نقاط اتصال نحاسية عند التقاء حجرين
        """
        TW  = SVGRenderer.TW      # 100
        TH  = SVGRenderer.TH      # 50
        GAP = SVGRenderer.CHAIN_GAP  # 0
        ...
        chain = []            # [(tile, is_double, width_on_chain)]
        total_chain_w = 0
        has_double = False
        ...
        for item in played:
            ...
            is_dbl = (tile.a == tile.b)
            # ★ الدبل أضيق على السلسلة (TH بدل TW)
        ...
        total_chain_w -= GAP   # حذف الفراغ الأخير
        if total_chain_w < 0:
            total_chain_w = 0

        # ── حساب أبعاد اللوحة ──
        # الدبل ارتفاعه TW=100 → نحتاج ارتفاعاً أكبر
        ...
        # ══ رسم سلسلة الأحجار (مجموعة واحدة بظل واحد) ══
        start_x = (full_w - total_chain_w) // 2
        svg += '<g filter="url(#chainShadow)">\n'

        cx = start_x
        junction_xs = []       # مواقع x لنقاط الاتصال

        for i, (tile, is_dbl, tw) in enumerate(chain):
            if is_dbl:
                # ★ دبل عمودي: يبرز فوق وتحت خط السلسلة
                ty = mid_y - TW // 2      # ارتفاع الدبل = TW (100)
                svg += SVGRenderer._draw_chain_tile(
                    tile, cx, ty, vertical=True
                )
            else:
                # عادي أفقي: متمركز على خط السلسلة
                ty = mid_y - TH // 2
                svg += SVGRenderer._draw_chain_tile(
                    tile, cx, ty, vertical=False
                )

            # ★ حفظ نقطة الاتصال (عند يمين الحجر الحالي)
            if i < n - 1:
                junction_xs.append(cx + tw)

            cx += tw + GAP

        svg += '</g>\n'   # إغلاق مجموعة الظل

        # ══ ★ نقاط الاتصال النحاسية بين الأحجار ══
        for jx in junction_xs:
    ...
    def analysis_chart(all_moves):
        ...
        for i, md in enumerate(all_moves[:n]):
            ...
            if   wr >= 0.6: color = "#4CAF50"
            elif wr >= 0.4: color = "#FFC107"
            else:           color = "#F44336"
