...
# ═══════════════════════════════════
    ...
    st.markdown("### 🎯 الطاولة")
    # ✅ تم إصلاح خطأ الـ w هنا لتتوافق مع محرك الرسم الجديد المترابط
    ...
    if turn == Pos.ME:
        ...
        with c1:
            next_enemy = Pos.RIGHT # الخصم القادم دائماً بعدي هو اليمين
        ...
        with c2:
            ...
            # تحليل ما يحبه الشريك
            prefs = Counter()
            for t in partner_played:
                prefs[t.a] += 1
                if not t.is_double: prefs[t.b] += 1
                ...
                for tab, pos in zip(tabs, [Pos.RIGHT, Pos.PARTNER, Pos.LEFT]):
                    with tab:
                        ...
                        for tile, prob in r['likely'][:8]:
                            ...
                            with c1: st.write(f"**{tile}**")
                            with c2: st.progress(min(prob, 1.0), text=f"{prob:.0%}")
        ...
        if real:
            ...
            for i, m in enumerate(real):
                with bcols[i % len(bcols)]:
                    ...
                    # تحليل خطر سريع لكل زر
                    btn_text = f"{'⭐' if is_rec else ''} [{m.tile.a}|{m.tile.b}] {d}"
                    if st.button(btn_text, key=f"m_{i}_{m.direction.value}", use_container_width=True, type="primary" if is_rec else "secondary"):
                        if gs.turn != Pos.ME:
        ...
        if pending is not None:
            ...
            with c1:
                if st.button("⬅️ في اليسار", key=f"pl_{btn_key}", use_container_width=True, type="primary"):
                    if apply_opponent_move(gs, turn, pending, Direction.LEFT): st.rerun()
            with c2:
                if st.button("في اليمين ➡️", key=f"pr_{btn_key}", use_container_width=True, type="primary"):
                    if apply_opponent_move(gs, turn, pending, Direction.RIGHT): st.rerun()
            st.markdown("")
            ...
            if gs.board.is_empty: playable = remaining
            else: playable = [t for t in remaining if gs.board.can_play(t)]
            
            if playable:
                playable.sort(key=lambda t: (t.a, t.b), reverse=True)
                
                # استخدام XRay لتلوين الأحجار المؤكدة التي قد يلعبها الخصم
                xray = XRayEngine(gs)
                ...
                for row_start in range(0, len(playable), ncols):
                    ...
                    for j, t in enumerate(row_tiles):
                        with cols[j]:
                            # تمييز الأحجار المتوقعة بوضع نجمة واسم براق
                            ...
                            if st.button(btn_label, key=f"o_{t.a}_{t.b}_{turn.value}", use_container_width=True):
                                if gs.board.is_empty:
                                    if apply_opponent_move(gs, turn, t, Direction.LEFT): st.rerun()
                                else:
                                    dirs = gs.board.can_play(t)
                                    if len(dirs) == 1:
                                        if apply_opponent_move(gs, turn, t, dirs[0]): st.rerun()
                                    elif len(dirs) >= 2:
                                        S('pending_tile', t)
                                        st.rerun()
                                    else:
...
# ═══════════════════════════════════
    ...
    if not is_locked:
        ...
        if win:
            st.balloons()
            st.markdown('<div style="text-align:center;padding:40px;background:linear-gradient(135deg,#1B5E20,#4CAF50);border-radius:18px;color:#fff;margin:20px 0"><h1>🏆 مبروك! الانتصار الساحق!</h1></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="text-align:center;padding:40px;background:linear-gradient(135deg,#B71C1C,#E53935);border-radius:18px;color:#fff;margin:20px 0"><h1>😔 خسارة مشرفة</h1></div>', unsafe_allow_html=True)
    else:
        st.markdown('<div style="text-align:center;padding:40px;background:linear-gradient(135deg,#E65100,#FF9800);border-radius:18px;color:#fff;margin:20px 0"><h1>🔒 اللعبة مقفولة! لنحسب النقاط..</h1></div>', unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1: st.metric("🟢 نقاط يدك (المتبقية)", f"{my_pts}")
    with c2: st.metric("❓ مجموع الخصوم والشريك", f"{others_total}")
        
    st.markdown("#### 🔢 أدخل أوراق الخصم لمعرفة النتيجة الحاسمة:")
    cc1, cc2 = st.columns(2)
    with cc1:
        opp_pts = st.number_input("🔴 أوراق الخصوم (يمين + يسار):", min_value=0, max_value=others_total, value=min(others_total, 10))
    with cc2:
        default_part = max(0, others_total - opp_pts)
        part_pts = st.number_input("🔵 أوراق شريكك:", min_value=0, max_value=others_total, value=default_part)
