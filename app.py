...
# ═══════════════════════════════════════════════════════
# 🧮 عدّاد القطع الاستراتيجي (النسخة المصححة والنظيفة)
# ═══════════════════════════════════════════════════════
class TileCounter:
    TOTAL_PER_NUMBER = 7
    ...
    def _count_all(self):
        # 1. إحصاء قطع الطاولة بدقة (معاملة الدبل كقطعة واحدة)
        for t in self.gs.board.tiles_on_table:
            self.on_board[t.a] += 1
            if not t.is_double:
                self.on_board[t.b] += 1
                
        # 2. إحصاء قطع اليد بنفس المنطق
