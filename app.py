...
def apply_opponent_move(gs, turn, tile, direction):
    ...
    # حفظ العداد قبل التطبيق لمنع الإنقاص المزدوج
    count_before = gs.players[turn].count
    ok = gs.apply(m)
    if not ok:
        show_message("❌ فشل تطبيق الحركة داخلياً!", "error")
        return False
    # إنقاص العداد فقط إذا apply لم تفعل ذلك تلقائياً
...
# ═══════════════════════════════════
            ...
            for i, m in enumerate(real):
                with bcols[i % len(bcols)]:
                    ...
                    if st.button(
                        ...
                        # ✅ إصلاح: تحقق من الدور قبل التطبيق
                        if gs.turn != Pos.ME:
    ...
    # ═══════════════════════════════════
    # ✅ يدي + السجل — خارج if/else ليظهرا دائماً
    # ═══════════════════════════════════
    st.markdown("---")
    st.markdown("### 🃏 أحجارك")
    playable_idx = []
    if not gs.board.is_empty:
        ...
        # الطاولة فارغة → كل الأحجار قابلة للعب
        playable_idx = list(range(len(gs.my_hand)))
    # التوهج فقط في دورك أنت
    if turn != Pos.ME:
        playable_idx = []
